...
export async function handleMock(config: AxiosRequestConfig) {
  ...
  if (url.includes('/donations') && method === 'get') {
    ...
    if (params.distanceKm && params.distanceKm > 0) {
      // Casablanca center coordinates (default user location)
      const userLat = 33.5731;
      const userLng = -7.5898;
      ...
      filtered = filtered.filter(d => {
        ...
        const dLat = toRad(d.latitude - userLat);
        const dLng = toRad(d.longitude - userLng);
        
        const a = 
          Math.sin(dLat / 2) * Math.sin(dLat / 2) +
          Math.cos(toRad(userLat)) *
            Math.cos(toRad(d.latitude)) *
            Math.sin(dLng / 2) *
            Math.sin(dLng / 2);
        ...
        return distance <= params.distanceKm;
      });
    }
